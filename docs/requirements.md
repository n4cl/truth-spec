...
- **Single Truth per Spec Kind**
  ...
  - システム設計書は `docs/system-design.md` の 1 ファイルが真となる。
  - UI / 画面設計書は `docs/ui-design.md` の 1 ファイルが真となる（必要な場合）。
  - 同じ役割のドキュメントを複数作らず、要件や設計が増えたときは既存ファイルを更新する。
...
- プロジェクトの「真の仕様書」として使う Markdown テンプレートを役割ごとに提供する。
- 最低限提供するテンプレート：
  - `docs/requirements-template.md` → `docs/requirements.md` 用
  - `docs/system-design-template.md` → `docs/system-design.md` 用
...
**F-001-2: system-design.md テンプレ**

- **基本設計＋詳細設計を 1 冊にまとめるイメージ**で構成する。
- 含めるセクションの例：
  1. アーキテクチャ概要（構成図・コンポーネント一覧）
  2. データフロー / シーケンス概要
  3. コンポーネント設計（役割・入出力・依存関係）
  4. データモデル / スキーマ概要
  5. 外部インターフェース（API / 外部サービス連携など）
  6. 詳細設計（主要な処理やアルゴリズムの説明）
...
- LLM に「truth-spec を読んで変更案を反映する文章を提案して」と依頼するための **プロンプトテンプレート**を提供する。
- 用途別に最低 3 種類を想定：
  - 新しい機能要件を追加するとき（requirements 用）
  - 既存の仕様を変更するとき（requirements / system-design 共通）
  - 実装変更に伴う設計セクションの追従をしたいとき（system-design 用）
...
- `docs/s-sdd-guide.md` として以下をまとめたガイドを提供する：
  - truth-spec（`requirements.md` / `system-design.md` など）に書くべき内容
...
- truth-spec 導入ステップは、基本的に以下の **最小 3 ステップ** に収まること：
  ...
  2. プロジェクト内に `docs/requirements.md` / `docs/system-design.md` を作る（テンプレからコピー）
...
- Git の diff から：「この変更はどの truth-spec（requirements / system-design 等）のどのセクションに影響しそうか？」をエージェントが指摘する機能。
...
- P01-02: 真の仕様書（`docs/requirements.md` / `docs/system-design.md` 等）を空 or 最小構成の状態から開始できること。ビジョン・対象ユーザー・初期スコープだけを書いてスタートし、段階的にユースケース・要件・設計を埋めていけること。
- P01-03: 「新規プロジェクトではこう使う」と分かる導入手順をガイドとして提供すること。`docs/s-sdd-guide.md` 内に「新規プロジェクト用セットアップ」セクションを用意する。
- P01-04: 新規プロジェクトにおいて、truth-spec が「最初に作る技術ドキュメント」として位置づけられるよう、デフォルトで `docs/requirements-template.md` / `docs/system-design-template.md` をわかりやすく提示すること。
...
- P02-01: 既存リポジトリに対しても、破壊的変更なく truth-spec を追加できること。`docs/requirements.md` / `docs/system-design.md` を新規作成するだけで基本が成立すること。
