...
- **「1サービス1ドキュメント」が原則だが、サイズ肥大時のみ分割可**
  - `requirements.md` を親（真の所在）とし、必要に応じて `docs/requirements/` 配下にサブファイルを置く。親から必ずリンクし、範囲を明記する。
  - 分割は例外措置とし、機能ごとではなく性質・断面（例: API 詳細、特定ドメインの大口機能）でのみ切る。
  - 目安：本文が 800–1000 行 / 150–200KB 付近、またはモデルコンテキスト上限の 60–70% を超え始めたら検討する。
